...
for lbl_Indx = 1 : NUM_LABELS
    ...
    bar(label-1, nnz(double(Y_pred)==label),0.4 ,"stacked", "DisplayName", "Predicted","EdgeColor","k","FaceColor",'r')
    bar(label-1+0.4, nnz(double(Y)==label),0.4, "stacked", "DisplayName", "Real","EdgeColor","k","FaceColor",'g')
...
xticks(labels-ones(size(labels)))
